...
describe('IocContainer', () => {
  ...
  describe('Выполнение скопа в основном потоке', () => {
    it('должен регистрировать и разрешать зависимости в дефолтном scope', () => {
      ioc.resolve<ICommands>('IoC.Register', 'testCommand', (args: any[]) => {
    ...
    it('должен поддерживать передачу аргументов в resolve', () => {
      ...
      const commandWithArgs = ioc.resolve<MockCommand>('testCommand', 'customArg');
      expect(commandWithArgs.getValue()).toBe('customArg');
    });
    
    it('должен выбрасывать ошибку при отсутствии зависимости', () => {
      expect(() => ioc.resolve<any>('unknownKey')).toThrow('Зависимость не найдена: unknownKey');
    });
  });
    ...
    it('должен работать с воркером в новом scope', async () => {
      ...
      workerMock.mockImplementation(() => {
        const mock = {
          on: jest.fn((event, callback) => {
            if (event === 'message') {
              setTimeout(() => callback({ type: 'result', result: new MockCommand('worker') }), 0);
            }
            return mock;
          }),
          off: jest.fn(),
      ...
      const workerCommand = await ioc.executeInWorker<MockCommand>('workerScope', 'testCommand');
      expect(workerCommand.getValue()).toBe('worker');
      
      // Проверка вызова воркера
      expect(workerMock).toHaveBeenCalledWith(`${__dirname}/workerScopeWorker.js`, {
        workerData: { scopeId: 'workerScope' },
      });
    }, 10000);
    
    it('должен выбрасывать ошибку, если воркер не найден', async () => {
      await expect(ioc.executeInWorker('nonExistentScope', 'testCommand')).rejects.toThrow(
        'Worker для скопа nonExistentScope не найден'
      );
    });
  });
  ...
  describe('Переключение скопа', () => {
    ...
    it('должен выбрасывать ошибку при переключении на несуществующий scope', () => {
      expect(() => ioc.resolve<ICommands>('Scopes.Current', 'nonExistentScope').execute()).toThrow(
        'Скоп не найден: nonExistentScope'
      );
    });
  });
});
