...
describe('MoveService', () => {
	...
	it('Перемещаем объект из (12, 5) со скоростью (-7, 3) в (5, 8)', () => {
		...
		// Мокаем методы
		jest
			.spyOn(movableAdapter, 'getLocation')
			.mockImplementation(() => currentLocation);
		jest
			.spyOn(movableAdapter, 'getVelocity')
			.mockReturnValue(new VectorService(-7, 3));
		jest
			.spyOn(movableAdapter, 'setLocation')
	...
	it('Выдаем ошибку при вызове getLocation когда невозможно прочитать положение в пространстве', () => {
		...
		expect(() => movingAdapter.getLocation()).toThrow(
			'Невозможно прочитать положение в пространстве',
		);
	});

	it('Выдаем ошибку при вызове setLocation когда невозможно изменить положение в пространстве', () => {
		const movingAdapter = new MovingAdapter();
		expect(() => movingAdapter.setLocation(undefined as any)).toThrow(
			'Невозможно изменить положение в пространстве: position is undefined',
		);
	});
	...
	it('Выдаем ошибку при вызове getVelocity когда невозможно прочитать значение мгновенной скорости', () => {
		...
		expect(() => movingAdapter.getVelocity()).toThrow(
			'Невозможно прочитать значение мгновенной скорости',
		);
	});
});
