		...
		parentPort.on('message', (msg) => {
			...
			ioc
				.resolve<ICommands>('IoC.Register', 'MovingAdapter', (args: any[]) => {
					...
					const location = ioc.resolve<VectorService>(
						'VectorService',
						...locationArgs,
					);
					const velocity = ioc.resolve<VectorService>(
						'VectorService',
						...velocityArgs,
					);
			...
			const movingAdapter = ioc.resolve<MovingAdapter>(
				'MovingAdapter',
				...msg.args,
			);

			// Создаем MoveService с адаптером
			const moveService = ioc.resolve<MoveService>(
				'MoveService',
				...[movingAdapter],
			);
