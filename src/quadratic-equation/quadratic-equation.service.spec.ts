...
describe('Решение квадратных уравнений', () => {
  ...
  it('Если коэффициент "a" равен нулю. Выбрасывать ошибку', () => {
    expect(() => service.solve(0, 1, 1)).toThrowError('Коэффициент "a" не может быть равен нулю.');
  });
  ...
  it('Выбрасываем ошибку для некорректных коэффициентов', () => {
    ...
    invalidValues.forEach((value) => {
      expect(() => service.solve(value, 1, 1)).toThrowError('Коэффициенты должны быть числами.');
      expect(() => service.solve(1, value, 1)).toThrowError('Коэффициенты должны быть числами.');
      expect(() => service.solve(1, 1, value)).toThrowError('Коэффициенты должны быть числами.');
    });
  });
});
