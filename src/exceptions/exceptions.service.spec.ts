...
describe('QueueCommandService (NestJS DI)', () => {
  ...
  it('Повторяем команду 1 раз после пишем ошибку в лог', () => {
    ...
    exceptionService.register('TestCommand', 'Error', (cmd, error) => {
      queueService.add(new RetryCommandService(cmd));
    });
    exceptionService.register('RetryCommandService', 'Error', (cmd, error) => {
  ...
  it('Повторяем команду 2 раз после пишем ошибку в лог', () => {
    ...
    exceptionService.register('RetryTwoCommandService', 'Error', (cmd, error) => {
      queueService.add(new LogCommandService(error));
    });
