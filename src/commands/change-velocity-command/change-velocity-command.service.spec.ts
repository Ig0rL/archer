...
describe('ChangeVelocityCommandService', () => {
  ...
  beforeEach(async () => {
    ...
    changeVelocityCommandService = module.get<ChangeVelocityCommandService>(ChangeVelocityCommandService);
    changeVelocityCommandAdapter = module.get<ChangeVelocityCommandAdapter>(ChangeVelocityCommandAdapter) as jest.Mocked<ChangeVelocityCommandAdapter>;
  });
  ...
  it('должен корректно вычислять новую скорость', () => {
    ...
    changeVelocityCommandAdapter.getCurrentLocation = jest.fn().mockReturnValue(currentLocation);
    ...
    const expectedVelocity = {
      x: Math.round(currentLocation.x * Math.cos((currentAngel * Math.PI) / 180) - currentLocation.y * Math.sin((currentAngel * Math.PI) / 180)),
      y: Math.round(currentLocation.x * Math.sin((currentAngel * Math.PI) / 180) + currentLocation.y * Math.cos((currentAngel * Math.PI) / 180)),
    };
    
    expect(changeVelocityCommandAdapter.getVelocity()).toEqual(expectedVelocity);
  });
});
