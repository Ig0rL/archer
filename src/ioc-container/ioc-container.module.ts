...
export class IocContainerModule implements OnModuleInit {
  constructor(private readonly ioc: IocContainer, private worker: FastMoveScopeWorker) {
    // Регистрация VectorService
    this.ioc.resolve<ICommands>('IoC.Register', 'VectorService', (args: any[]) => {
      const [x, y] = args;
      return new VectorService(x, y);
    }).execute();
    
    // Регистрация MovingAdapter
    this.ioc.resolve<ICommands>('IoC.Register', 'MovingAdapter', (args: any[]) => {
      const [locationArgs, velocityArgs] = args
      const location = this.ioc.resolve<VectorService>('VectorService', ...locationArgs);
      const velocity = this.ioc.resolve<VectorService>('VectorService', ...velocityArgs);
      return new MovingAdapter(location, velocity);
    }).execute();
    
    // Регистрация MoveService в дефолтном scope
    this.ioc.resolve<ICommands>('IoC.Register', 'MoveService', (args: any[]) => {
  ...
  async onModuleInit(): Promise<void> {
    const adapterArgs = [[12, 5], [-7, 3]];
    const movingAdapter = this.ioc.resolve<MovingAdapter>('MovingAdapter', ...adapterArgs);
    
    // Создаем MoveService с адаптером
    const moveService = this.ioc.resolve<MoveService>('MoveService', ...[movingAdapter]);
    ...
    const fastMoveResult = await this.ioc.executeInWorker('fastMoveScope', 'MoveService', ...adapterArgs);
